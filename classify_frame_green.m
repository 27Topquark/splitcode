...
function [ ] = classify_frame_green( framestate,i,j,condition )
make_global
% INPUT ARGUMENTS
% framestate - frame state number that tells whether the frame on the
              ...
              case 1
                  if (strcmp('F',frame_state.tag(j)))
                     ...
                     if delta_X <= 50
                       ...
                       if (abs(centres(1,1) - 451) < abs(Front(j,1) - 451))
                           Front(i,:) = centres;
                           frame_state.number(i) = 1;
                           frame_state.tag(i) = 'F';
                     ...
                     if delta_X <= 50
                       ...
                       %either the foot has been lifted and put forward
                       if (abs(centres(1,1) - 1) > abs(Hind(j,1) - 1))
                           Hind(i,:) = centres;
                           frame_state.number(i) = 1;
                           frame_state.tag(i) = 'H';
